...
namespace OpenNos.Master.Server
{
    internal class CommunicationService : ScsService, ICommunicationService
    {
        ...
        public string RetrieveRegisteredWorldServers(string username, int sessionId, bool ignoreUserName, string regionCode)
        {
            if (!MSManager.Instance.AuthentificatedClients.Any(s => s.Equals(CurrentClient.ClientId)))
            {
                return null;
            ...
            string lastGroup = "";
            byte worldCount = 0;
            string channelPacket = $@"NsTeST  {regionCode} {(ignoreUserName ? "" : " " + username)} {sessionId} ";
