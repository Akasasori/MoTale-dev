        ...
        public DeleteResult DeleteByPrimaryKey(long accountId, byte characterSlot)
        {
            try
            {
                using (OpenNosContext context = DataAccessHelper.CreateContext())
                {
                    ...
                    if (character != null)
                    {
                        if (character.FamilyCharacter == null)
                        {
                            character.State = (byte)CharacterState.Inactive;
                            character.Name = $"[DELETED]{character.Name}";
                            context.SaveChanges();
                        }
                        else if (character.FamilyCharacter != null)
                        {
                          // character.DAOFactory.FamilyCharacterDAO.Delete(Session.Character.CharacterId);

                            character.LastFamilyLeave = DateTime.Now.Ticks;
                            character.FamilyCharacter = null;
                            character.State = (byte)CharacterState.Inactive;
                            character.Name = $"[DELETED]{character.Name}";
                            context.SaveChanges();
                        }
