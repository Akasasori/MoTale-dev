    ...
    public class AccountDAO : IAccountDAO
    {
        ...
        public void ResetDailyRewards()
        {
            try
            {
                using (OpenNosContext context = DataAccessHelper.CreateContext())
                {
                    foreach (Account s in context.Account)
                    {
                        if (s.DailyRewardSent)
                        {
                            AccountDTO accountDTO = new AccountDTO();
                            s.DailyRewardSent = false;
                            if (Mapper.Mappers.AccountMapper.ToAccountDTO(s, accountDTO))
                            {
                                InsertOrUpdate(ref accountDTO);
                            }
                        }
                    }
                }
            ...
            {
                Logger.Error(Language.Instance.GetMessageFromKey("RESET_DAILY_REWARDS_ERROR"), e);
            }
        }
