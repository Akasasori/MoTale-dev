    ...
    public class MinilandPacketHandler : IPacketHandler
    {
        ...
        public void JoinMiniland(MJoinPacket mJoinPacket)
        {
            if (Session.Character.MapInstance.MapInstanceType == MapInstanceType.BaseMapInstance || Session.Character.IsSeal)
            {
                ...
                if (sess?.Character != null && (ServerManager.Instance.ChannelId != 51 || sess.Character.Faction == Session.Character.Faction))
                {
                    ...
                    if (sess.Character.MinilandState == MinilandState.Open || sess.Character.MinilandState == MinilandState.Private && sess.Character.IsFriendOfCharacter(Session.Character.CharacterId))
                {
        ...
        public void MinilandEdit(MLEditPacket mlEditPacket)
        {
            if (mlEditPacket != null && mlEditPacket.Parameters != null)
            {
                ...
                {
                    case 1:
                        string intro = mlEditPacket.Parameters.Replace(' ', '^');
                        Session.SendPacket($"mlintro {intro}");
                        Session.SendPacket(
                            UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("MINILAND_INFO_CHANGED")));
                        Session.Character.MinilandMessage = intro;
                        break;

                    case 2:
                        if (Enum.TryParse(mlEditPacket.Parameters, out MinilandState state))
                        {
                            switch (state)
                            {
                                case MinilandState.Private:
                                    ...
                                    Session.Character.Miniland.KickAll(Session.Character, true);
                                    break;
                                ...
                                case MinilandState.Lock:
                                    Session.SendPacket(
                                        UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("MINILAND_LOCK"),
                                            0));
                                    Session.Character.Miniland.KickAll(Session.Character);
                                    break;
